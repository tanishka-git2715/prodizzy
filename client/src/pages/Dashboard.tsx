...
function matchScore(p: StartupProfile): number {
  const fields = [
    ...
    p.num_users, p.monthly_revenue, p.traction_highlights,
    p.website
  ];
        ...
        <div className="max-w-6xl mx-auto px-6 py-10 space-y-10">
          ...
          <div>
            ...
            <p className="text-white/35 mt-1 text-sm">
              {profile.company_name} · {profile.role} · {profile.stage}
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            ...
            <div className="lg:col-span-2 bg-white/[0.03] border border-white/8 rounded-2xl p-6 space-y-5">
              <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-6">
                ...
                <div className="shrink-0 flex items-center gap-3 bg-white/[0.03] px-4 py-3 rounded-2xl border border-white/5">
                  <div className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-white font-medium border border-white/10 text-xs shadow-[inset_0_1px_1px_rgba(255,255,255,0.05)]">
              ...
              {(profile.num_users || profile.monthly_revenue) && (
                <div className="flex flex-wrap gap-4 pt-3 border-t border-white/6 text-xs">
                  {profile.num_users && (
                    <span className="text-white/40">Users: <span className="text-white/70">{profile.num_users}</span></span>
                  )}
                  {profile.monthly_revenue && (
                    <span className="text-white/40">Revenue: <span className="text-white/70">{profile.monthly_revenue}</span></span>
                  )}
                </div>
              )}
            </div>
            ...
            <ProCard
              ...
              onSave={() => patchMutation.mutate({
                team_size: (teamSize || profile.team_size || undefined) as any,
                missing_roles: missingRoles.length ? missingRoles : profile.missing_roles || undefined,
                hiring_urgency: (hiringUrgency || profile.hiring_urgency || undefined) as any,
              })}
